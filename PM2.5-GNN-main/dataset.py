...
class HazeData(data.Dataset):
    ...
    def _process_feature(self):
        ...
        # 确保使用正确的节点数
        node_num = self.feature.shape[1]  # 使用实际的节点数，而不是graph.node_num
        ...
        # 使用实际的节点数进行重复
        h_arr = np.repeat(h_arr[:, None], node_num, axis=1)
        w_arr = np.repeat(w_arr[:, None], node_num, axis=1)
